...
public class FolderBuilder {
    ...
    private String zipDirectoryPath = "./finalThing";
    private String directoryPath = "./finalThing";
    ...
    public void StructureBuild() {
        try {
            FileUtils.forceMkdir(new File(directoryPath + "/"+ name.toString()));
            directoryPath = directoryPath +  "/"+ name.toString();
            System.out.println("第一層資料夾創建成功");

            for (int i = 1; i <= num; i++) {
                String FileRoot = directoryPath + "/"+ name.toString() + "-" + i;
    ...
    public void Compressor(){
        zipDirectoryPath = "./finalThing/" + name + ".zip";
        try {
    ...
    public void CFileWriter(String list,int i){
        String filePath = list + "/" + name + "-" + (i + 1) + ".c";

        StringBuilder cCode = new StringBuilder();
        System.out.println("目前正在處理第" + (i + 1) + "檔案 請輸入cCode 若已輸入完成 請輸入0作結束");
